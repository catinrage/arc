...
ARC_SERVICE_NAMESPACE="${ARC_SERVICE_NAMESPACE:-}"
ARC_UPDATE_REPO="${ARC_UPDATE_REPO:-catinrage/arc}"
ARC_UPDATE_API_URL="${ARC_UPDATE_API_URL:-https://api.github.com/repos/${ARC_UPDATE_REPO}/releases}"
ARC_UPDATE_INCLUDE_PRERELEASE="${ARC_UPDATE_INCLUDE_PRERELEASE:-1}"
...
Usage:
  ...
  ./service.sh init <agent|gateway>
  ./service.sh <agent|gateway> <init|start|stop|restart|status|enable|disable|stateus>

Examples:
  ./service.sh update
  ./service.sh status
  ./service.sh init agent
  ./service.sh gateway init
  ./service.sh agent restart
  ./service.sh gateway status

Notes:
  - 'update' downloads the newest GitHub release and updates release-managed files.
  - 'update' preserves local agent/gateway config JSON files.
  - 'init' creates the systemd unit, enables it, and starts it.
  - Top-level start/stop/restart operate on initialized Arc services.
  - Set ARC_SERVICE_NAMESPACE=name to keep service metadata and defaults separate.
  - Set ARC_SERVICE_NONINTERACTIVE=1 to accept default service names/descriptions.
  - Set ARC_UPDATE_REPO=owner/repo if this repository is forked.
...
current_install_version() {
    ...
    for binary in arc-agent arc-gateway; do
...
for release in releases:
    ...
    for asset in release.get("assets", []):
        ...
        if name.startswith("arc_") and name.endswith("_linux_amd64.tar.gz"):
            tar_asset = asset
        elif name.startswith("arc_") and name.endswith("_linux_amd64.tar.gz.sha256"):
            sha_asset = asset
...
copy_release_tree() {
    ...
    (cd "$release_root" && find . -mindepth 1 -print0) |
        while IFS= read -r -d '' rel; do
            rel="${rel#./}"
            case "$rel" in
                config.agent.json|config.gateway.json|agent.json|gateway.json)
...
validate_role() {
    case "$1" in
        agent|gateway) printf '%s' "$1" ;;
        *) fail "invalid role '$1'; expected 'agent' or 'gateway'" ;;
    esac
}
...
role_key() {
    local role="$1"
    local ns
    ns="$(sanitize_namespace "$ARC_SERVICE_NAMESPACE")"
    if [[ -n "$ns" ]]; then
        printf '%s-%s' "$role" "$ns"
    else
        printf '%s' "$role"
...
set_role_vars() {
    ...
    case "$role" in
        agent)
            ROLE_BINARY="arc-agent"
            ROLE_CONFIG="config.agent.json"
            ROLE_CONFIG_EXAMPLE="agent.example.json"
            ROLE_DESCRIPTION_DEFAULT="Arc Agent"
            ROLE_SERVICE_DEFAULT="$(namespaced_default_service arc-agent)"
            ;;
        gateway)
            ROLE_BINARY="arc-gateway"
            ROLE_CONFIG="config.gateway.json"
            ROLE_CONFIG_EXAMPLE="gateway.example.json"
            ROLE_DESCRIPTION_DEFAULT="Arc Gateway"
            ROLE_SERVICE_DEFAULT="$(namespaced_default_service arc-gateway)"
            ;;
    esac
...
prompt_with_default() {
    ...
    if [[ "${ARC_SERVICE_NONINTERACTIVE:-0}" == "1" ]]; then
...
top_level_start() {
    ...
    [[ "$found" -eq 1 ]] || printf 'No initialized Arc services found.\n'
}
...
top_level_stop() {
    ...
    [[ "$found" -eq 1 ]] || printf 'No running Arc services found.\n'
}
...
init_service() {
    ...
    if [[ ! -f "$ROLE_WORKDIR/$ROLE_CONFIG" ]]; then
        [[ -f "$ROLE_WORKDIR/$ROLE_CONFIG_EXAMPLE" ]] || fail "config file not found: $ROLE_WORKDIR/$ROLE_CONFIG"
        cp "$ROLE_WORKDIR/$ROLE_CONFIG_EXAMPLE" "$ROLE_WORKDIR/$ROLE_CONFIG"
        printf 'Created %s from %s. Edit it before production use.\n' "$ROLE_CONFIG" "$ROLE_CONFIG_EXAMPLE"
    fi
...
ExecStart=$ROLE_BINARY_PATH -config $ROLE_CONFIG
Restart=always
RestartSec=5
LimitNOFILE=1048576
...
EOF
    ...
    printf 'Manage with: ./service.sh %s <status|start|stop|restart|enable|disable>\n' "$role"
}

top_level_init() {
    local target="$1"
    set_role_vars "$target"
    init_service "$target"
    enable_and_start_service "$target"
}
...
stop_active_services() {
    ...
    if [[ ${#services[@]} -eq 0 ]]; then
        printf 'No active Arc systemd services were detected before update.\n'
        return
    fi
...
update_install() {
    ...
    download_file "$ARC_UPDATE_API_URL" "$releases_json"

    release_line="$(select_update_release "$releases_json" "$ARC_UPDATE_INCLUDE_PRERELEASE")"
    ...
    chmod +x "$SCRIPT_DIR/arc-agent" "$SCRIPT_DIR/arc-gateway" "$SCRIPT_DIR/service.sh" 2>/dev/null || true
